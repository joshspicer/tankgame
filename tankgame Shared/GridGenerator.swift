...
struct GridGenerator {
    static func generate(seed: UInt32) -> [[GridCell]] {
        ...
        let destructibleRatio = 0.4 // 40% of walls are destructible
        ...
        for row in 0..<8 {
            for col in 0..<8 {
                ...
                if !protectedCells.contains(key) && !borderCells.contains(key) && rng.nextDouble() < wallDensity {
                    // Decide if this wall should be destructible
                    if rng.nextDouble() < destructibleRatio {
                        grid[row][col] = .destructibleWall
                    } else {
                        grid[row][col] = .wall
                    }
                }
