...
class GameScene: SKScene {
    ...
    func startGame(with state: GameState) {
        ...
        ui.hidePlayAgainButton() // Hide button when starting new round
        renderGrid()
        renderTanks()
        updateScore()
    ...
    func handlePlayAgain() {
        guard let state = gameState else { return }
        ui.showWaitingForPlayers()
        soundManager.playSound("shoot.wav") // Play a click sound
            ...
            if state.isRoundOver() {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak self] in
                        ...
                        self.ui.showPlayAgainButton(in: self, sceneSize: self.size)
                    }
                }
            }
...
extension GameScene {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if ui.isPlayAgainButtonTouched(at: location) {
                handlePlayAgain()
                continue
            }
